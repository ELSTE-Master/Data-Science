...
We have reviewed some descriptive parameters of distributions. Some parameters are **parametric**, which means that they [imply an underlying type of distribution]{.mark} (e.g., the [mean](#mean) and [standard deviation](#variance-and-standard-deviation) imply a **normal** distribution). Other parameters are **non-parametric**, and are in theory valid for [any type of distribution]{.mark}. For instance, the [IQR](#interquartile-range) provides a better representation of the range of asymmetrical distributions. However, it [still represents a range that does not convey the shape of the underlying distribution]{.mark}, so reporting a few percentiles (e.g., 10-90, 25-75) typically better convey an idea of spread and distribution shape.
