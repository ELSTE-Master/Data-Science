...
:::: {.columns}
::: {.column width="52%"}
**Descriptive parameters**: capture the properties of the distribution

- Center of mass → [location]{.bg}
- Variability → [dispersion]{.bg}
- Shape → [skewness]{.bg}
...
The **standard deviation** ($\sigma$) is the sum of squares differences between data points $x_i$ and the
...
- $\sigma$ is in [the same unit as the data]{.bg}
  - Easy to interpret
- `df.std()`
...
:::: {.columns}
::: {.column width="48%"}
It is closely related to the **normal distribution** as:
...
The **variance** ($\sigma$) is the [average of squared deviations]{.bg} from the mean (→ the square of the standard deviation):
$$
...
- $\sigma^2$ is in **not** the same unit as the data
...
The **skewness** measures [the asymmetry of a distribution]{.bg} and is computed as @eq-skew:
...
Beyond **histograms** → some plot types report indications of **location**, **dispersion** and **skewness**.
