...
- The [covariance]{.bg} captures **the direction and magnitude of the linear relationship between the two variables**
- The [sample covariance]{.bg} is an **estimator** of the theoretical covariance:
...
**Problem**: depends on the [magnitudes]{.bg} of the variables, so it does not directly reflect the [strength]{.bg} of the relationship



## Correlation

- The [correlation coefficient]{.bg} provides **a normalized version** of the covariance
  - Ranges from $−1$ to $1$
  - Indicates both the direction and the strength of the linear relationship
- The [sample correlation]{.bg} is computed as:
...
- [Correlation matrix]{.bg} → square table showing the [pairwise correlations]{.bg} between all variables in a dataset
- Suppose we have $p$ variables $(X_1, X_2, \dots, X_p)$ → the [correlation matrix]{.bg} $\mathbf{C}$ is:
...
- [linear regressions]{.bg} attempt modelling this relationship
...
→ Further investigations [require dedicated stats packages]{.bg}
...
Linear regression → [do residuals show any structure]{.bg}?? → `sns.residplot()`
...
::: {.fragment }
Any type of [structure in residuals]{.bg} might reveal a **violation of linear regression assumptions**.
:::
...
- Go to [https://elste-master.github.io/Data-Science/](https://elste-master.github.io/Data-Science/)
  - Class 2 > [Bivariate analyses]{.bg}
  - Explore the structure of the glass geochemistry or [your]{.bg} dataset
  - Get used to the various functions

<iframe src="../class2/day2-4_bivariate.html" width="100%" height="400px" style="border:none;"></iframe>
